...
class IntentClustering():
   def __init__(self, function_ids, code_reference, IC_ALGO, IC_KVAL):
      ...
      self.data, self.v_data, self.doc_to_id = self._preprocess()

   def write_to_file(self, data):
      with open('clusters.json', 'w') as fp:
        json.dump(data, fp)

   def _preprocess(self, embedder="STrans"):
   ...
   def core_get_clusters(self, embedder="STrans", method='kmeans', n_clusters=10, eps=0.5, min_samples=5, n_jobs=-1):
      ...
      assert type(n_clusters) == np.number
      assert (type(eps) == np.number) and (0.001 <= eps <= 1.0)
      assert n_jobs in (None, -1)
      assert type(min_samples) == int
      
      self._preprocess(embedder=embedder)
